...
class Profile extends React.Component {
  ...
  loadPersonalities = () => {
    if (this.props.personalities) {
      return this.props.personalities.personality.insights.needs.map((need) => <li>{need.trait_id}</li>)
    } else {
      return null
    }
  }
  ...
  render() {
    return (
      <div style={{height: "100%"}}>
        {this.props.personalities.isLoaded ?
          <div>
            {this.loadPersonalities()}
            <Link to={`users/${this.props.authentication.user.id}`}> Link </Link>
          </div>
        :
          <Loading />
        }
